...
- No flakiness at 1-8 threads; all 5 flakes occur under 12/64 test threads,
  i.e. they are contention-sensitive, not time-bomb flakes.
- Tests 1-3 shared one pattern: waits on background matchers/seeded items
  bounded by fixed tick loops or by "any selectable item". All three are
  resolved: tests 1-2 now wait on target-specific Nucleo convergence, and
  test 3 is removed with its contract covered deterministically below the
  app layer.
- Test 4 (300s timeout) was removed as a redundant cross-layer harness; its
  cancellation and session-renewal contracts live in deterministic
  maki-ui/maki-commands tests. No internal blocking path was ever found, and
  the full remote CI suite passes without hangs after the removal.
- Test 5's root cause was the probe's unqualified first-stat gate, not a
  lock-scope gap in the memory tool; the gate is now armed per path, and the
  serialization guarantee holds under stress.
