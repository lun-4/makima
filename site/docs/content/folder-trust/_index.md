...
A project `.makima` directory can run code on your machine before you type
anything. Makima loads none of it until you trust the folder.

| Gated file | What it can do |
|------------|----------------|
| `.makima/.env` | sets environment variables, including secrets, for Maki and every process it starts |
| `.makima/permissions.toml` | decides which tools run without asking |
| `.makima/init.lua` | runs Lua inside Maki's own process at startup |
| `.makima/mcp.toml` | starts MCP servers as child processes |
...
exits Makima. A project that ships no gated file is never asked about.

One answer covers one project root: the active Git checkout, or the working
directory outside Git. Linked worktrees answer for themselves. Starting Makima in
your home directory loads no project configuration, because `~/.makima` there is
...
- Commands under `.makima/commands` and `.claude/commands`
- Skills under `.makima/skills`, `.claude/skills`, `.opencode/skills` and
  `.agents/skills`
...
The `deny` scopes in `.makima/permissions.toml` apply without trust. Its `allow`
...
Makima writes nothing into a folder you declined, and the status bar carries a
`[restricted]` indicator for the whole session. A folder with no `.makima` at all
shows no indicator.
...
`~/.config/makima/permissions.toml`, or trust the folder.

## Managing Trust

```bash
makima trust add [PATH]        # asks before recording
makima trust add [PATH] --yes  # records a yes
makima trust remove [PATH]     # clears a yes or a no
makima trust list              # shows both kinds of decision
```
...
project adds while Makima runs is asked about on the next start.
...
Makima reads `trust` from the global `~/.config/makima/init.lua` only. A project
`.makima/init.lua` that sets it has the table stripped and gets a warning, since a
...
so `makima trust list` shows it and `makima trust remove` clears it.
...
`makima trust remove PATH`, or `/trust` inside the TUI.
...
(`makima index`, `makima models`, `makima prompt`, `makima mcp auth`) skip it, since a
...
Makima records the file names rather than their contents, so Lua that changes in a
later pull runs under the answer you already gave. Run `makima trust remove` when
