...
impl CatalogData {
    fn from_index(
        ...
        for (provider_id, provider) in index {
            ...
            if builtin_provider(slug).is_some() && !OPENCODE_FAMILY_SLUGS.contains(&slug) {
...
fn opencode_catalog_config() -> (std::collections::HashSet<String>, bool) {
    ...
    let enable_free = maki_config::providers::ProvidersConfig::load()
        ...
        .unwrap_or(false);
    (all_slugs, enable_free)
}

pub(crate) fn init_shared_catalog_if_needed() -> &'static Mutex<CatalogData> {
    let (all_slugs, enable_free) = opencode_catalog_config();
...
pub fn refresh_catalog() -> Result<(), AgentError> {
    ...
    let (all_slugs, enable_free) = opencode_catalog_config();
    let data = fetch_catalog_blocking(&state_dir, all_slugs, enable_free)?;
...
fn parse_model(model: &schema::CatalogModel) -> CatalogMeta {
    ...
    CatalogMeta {
        context: limit.and_then(|l| l.context).filter(|tokens| *tokens > 0),
        output: limit.and_then(|l| l.output).filter(|tokens| *tokens > 0),
...
fn init_catalog_blocking(
    ...
    if let Some(index) = smol::block_on(load_cached_catalog_async()) {
        return CatalogData::from_index(index, enable_free_models, &state_dir, opencode_slugs);
    }

    match fetch_catalog_blocking(&state_dir, opencode_slugs, enable_free_models) {
        Ok(data) => data,
        Err(e) => {
...
mod tests {
    ...
    use crate::providers::{Timeouts, deepseek};
    ...
    fn supports_vision_falls_back_to_catalog_for_builtins(model_id: &str, expected: bool) {
        ...
        let _lock = super::seed_catalog_for_tests(index, state_dir);
    ...
    fn catalog_answers_only_for_models_the_static_table_misses() {
        let (_tmp, state_dir) = temp_state_dir();
        let _lock = super::seed_catalog_for_tests(builtin_catalog(), state_dir);
    ...
    fn builtins_keep_their_catalog_metadata(
        ...
        let data = CatalogData::from_index(
            single_provider_catalog(catalog_id, npm, api),
            false,
            &state_dir,
            std::collections::HashSet::new(),
        );
    ...
    fn catalog_miss_falls_back_to_family() {
        let (_tmp, state_dir) = temp_state_dir();
        let _lock = super::warm_empty_catalog_for_tests(state_dir);
